...
def main():
    ...
    try:
        ...
        with args.holdings.open(newline='', encoding='utf-8-sig') as stream:
            reader = csv.DictReader(stream)
            official = reader.fieldnames == ['Date','PERMNO','TICKER','COMPANY NAME','WEIGHT']
            for row in reader:
                if official:
                    row = {'target_month': row['Date'], 'permno': row['PERMNO'],
