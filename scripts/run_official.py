...
def main():
    ...
    parser.add_argument('--config', default='configs/official_full.json')
    ...
    source_files = [args.config,__file__,'scripts/run_ridge_baseline.py','src/models/official_linear_baseline.py','src/data/prepare_quant.py','src/data/build_samples.py','src/data/splits.py','src/utils/dates.py','src/training/metrics.py','requirements-baseline.txt','scripts/verify_official_source.py','tests/test_official_baseline.py']
    outputs = [p.name for p in out.iterdir() if p.is_file()]
    write_json(out/'manifest.json',{'command':shlex.join([sys.executable,'-m','scripts.run_official','--config',args.config]),'cwd':str(Path.cwd()),'python':platform.python_version(),
